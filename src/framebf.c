...
mBuf[5] = 600; // Set width as per image width
mBuf[6] = 600; // Set height to smaller of screen or image height
...
void scrollImage() {
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < imgWidth; x++) {
            if (y + currentY < imgHeight) {
                unsigned int index = (y + currentY) * imgWidth + x;
...
void delay(int milliseconds) {
    volatile long wait;
    long loop_per_milli = 100000;  // This value needs calibration

    while (milliseconds > 0) {
        for (wait = 0; wait < loop_per_milli; wait++) {
